...
from experiments.qwen.utils.chat import (
    add_messages_and_targets,
    estimate_token_lengths,
    make_inference_prompt,
)
from experiments.qwen.utils.configs import build_qwen3_default_config
from experiments.qwen.utils.training import EpochEvaluationCallback
...
def main():
    ...
    logger.info("Token length percentiles (fake tokenizer): p50=%d p95=%d p100=%d", int(pd.Series(lens).quantile(0.5)), int(pd.Series(lens).quantile(0.95)), int(lens.max()))
