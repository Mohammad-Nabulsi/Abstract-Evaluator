...
def apply_qwen3_chat_template(tokenizer, messages, add_generation_prompt: bool = False):
...
def build_formatting_func(tokenizer):
    def formatting_prompts_func(examples):
        ...
        for messages in examples["messages"]:
            text = apply_qwen3_chat_template(
                tokenizer,
                messages,
...
def estimate_token_lengths(df_part: pd.DataFrame, tokenizer, messages_col: str = "messages") -> np.ndarray:
    ...
    for msgs in df_part[messages_col].tolist():
        txt = apply_qwen3_chat_template(tokenizer, msgs, add_generation_prompt=False)
